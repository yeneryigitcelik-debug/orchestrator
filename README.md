...
| `devops`   | sonnet        | Docker, CI/CD, deploy, infra config |
...
Every role's skills live in `skills/<role>/*.md` and are appended to a worker's
system prompt when it spawns. Browse and edit the whole catalog — models, base
prompts, and per-role skills — from the panel's **▦ ROSTER** view.
...
skills/
  <role>/*.md         per-role skill library (managed from the ▦ ROSTER view)
scripts/
...
prisma/
  schema.prisma       Worker, Message, Scan, Finding, AuditEvent tables
```
